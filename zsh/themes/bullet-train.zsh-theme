...
prompt_context() {
  ...
  local _context=$(hostname)
  if [ -n "${SSH_CLIENT}" ]; then
    [[ -n "$_context" ]] && prompt_segment red $BULLETTRAIN_CONTEXT_FG "$_context"
  else
...
prompt_status() {
  local symbols
  symbols=()
  # [[ $RETVAL -ne 0 && $BULLETTRAIN_STATUS_EXIT_SHOW != true ]] && symbols+='\U1F621'
  ...
  if [[ -n "$symbols" && $RETVAL -ne 0 ]]; then
    ...
    # prompt_segment black $BULLETTRAIN_STATUS_FG "$symbols"
    prompt_segment black red "$symbols"
  elif [[ -n "$symbols" ]]; then
